...
public class AlipayForestMonitor {
     private static  String TAG="AlipayForestMonitor.class";
    ...
    public static void enterForestUI(AccessibilityNodeInfo nodeInfo) {
        Log.d(TAG, "enterForestUI ");
        if (nodeInfo != null) {
            // 找到界面中蚂蚁森林的文字
            List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByText("蚂蚁森林");

            if (list == null) {
                Log.d(TAG, "enterForestUI finding no");
                return;
            } else {
                Log.d(TAG, "enterForestUI finding yes");
            }
            ...
            if (nodeInfo != null) {
                for (int i = 0; i < nodeInfo.getChildCount(); i++) {
                    ...
                    if ("com.uc.webview.export.WebView".equals(child.getClassName())) {
                        Log.d(TAG, "找到蚂蚁森林的 webView count = " + child.getChildCount());
        ...
        if (null != node && node.getChildCount() > 0) {
            for (int i = 0; i < node.getChildCount(); i++) {
                ...
                if ("android.widget.Button".equals(className)) {
                    ...
                    boolean isResIdNull = false;
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
                        isResIdNull = child.getViewIdResourceName() == null ? true:false;
                    }
